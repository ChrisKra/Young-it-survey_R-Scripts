...
for(p in 1:length(desc)) {
  png(file=paste("Generated_Charts/CarAnch_Charts-", p, "-", str_replace_all(desc[p], "/", "_"), ".png", sep = ""),width=indWidth, height=indHeight)
  
  print(ggplot(grouped_list[[p]], aes(x=rank)) +
...
# Clean up 
rm(df_list, tmp_df, areaAlpha, colNum, colorM, colorW, e, indHeight, indWidth, lineSize, maxValue, nColumns, p, s, smoothingAmount, desc, desc_de, desc_en, i, grouped_list, mapping)