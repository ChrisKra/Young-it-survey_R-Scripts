...
s <- which(colnames(df) == "Wahrnehmung IT [Ich würde mich sehr freuen, den Rest meiner Karriere in der IT zu verbringen.]") # First column
e <- which(colnames(df) == "Indentifikation IT [Ich freue mich, dem IT-Beruf anzugehören.]") # Last column
...
png(file = "Generated_Charts/IT_Charts_Overview.png",width=(col*indWidth), height=(ceiling(nColumns/col)*indHeight), pointsize = 22)
