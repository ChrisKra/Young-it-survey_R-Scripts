...
s <- which(colnames(df) == "Autonomie [Ich will für meine eigene Karriere verantwortlich sein.]") # First column
e <- which(colnames(df) == "orga Stab [Ich ziehe es vor, in einem Unternehmen zu bleiben, mit dem ich vertraut bin, anstatt anderswo nach einer Beschäftigung zu suchen.]") # Last column
...
png(file = "Generated_Charts/PCBC_Charts_Overview.png",width=(col*indWidth), height=(ceiling(nColumns/col)*indHeight), pointsize = 22)
